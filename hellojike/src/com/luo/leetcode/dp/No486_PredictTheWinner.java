...
public class No486_PredictTheWinner {
     ...
     * 首先证明最优子结构性质。
     * 对于数组[1..n]中的子数组[i..j]，假设玩家1在子数组[i..j]中的拿法是最优的，即拿的分数比玩家2多出最多。
     ...
     * 对于两个数字的子数组,即j-i=1，dp[i][j]=abs(num[i]-num[j]),玩家1先手拿两个数中大的一个，
     * 所以玩家1一定比玩家2多两个数字差的绝对值，为最优拿法。
     * 对于j-i>1的子数组，如果玩家1先手拿了i，则玩家1手里有num[i]分，
     * 则玩家2一定会按照[i+1..j]这个子数组中的最优拿法去拿，
     * 于是玩家2此时手里相当于有dp[i+1][j]分，于是玩家1比玩家2多num[i]-dp[i+1][j]分。
     * 如果玩家1先手拿了j，则玩家1手里有num[j]分，
    ...
    public boolean PredictTheWinner(int[] nums) {
        ...
        for (int i = len-1; i >= 0; i--) {
            for (int j = i; j < len; j++) {
                dp[i][j]=Math.max(
                        nums[i]-(i==len-1?0:dp[i+1][j]),
                        nums[j]-(j==0?0:dp[i][j-1]));
            }
        }
    ...
    public static void main(String[] args) {
        No486_PredictTheWinner test=new No486_PredictTheWinner();
//        int[] nums={1, 5, 2};

        int[] nums={1, 5, 233, 7};
        boolean b = test.PredictTheWinner(nums);
